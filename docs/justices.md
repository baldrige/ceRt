...
| `X, J., delivered the opinion of the Court with respect to Parts I–IV–B, and an opinion with respect to Part IV–C, in which A, B, and C, JJ., joined.` | the Court's convention: joiners are listed only where fewer than all joined, so a Court clause with no `in which` of its own was joined by every participant not on the dissent side (Barrett v. United States, OT25, read as 5–0 before this) |
