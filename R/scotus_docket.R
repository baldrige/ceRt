...
get_scotus_case <- function(dkt) {
  ...
  docs <- html %>% 
    html_table(header = TRUE) %>% pluck(2) 
  links1 <- html %>% 
  ...
  case <- tibble(caption = title, dkt = dkt, date = date, lower = lower, lower_dkt = lower_dkt, lower_date = lower_date, parties = list(parties), events = list(docs))
  return(case)
}

parse_parties <- function(html) {
  ids <- html %>% html_table() %>% pluck(3, "X1") %>% str_which("Attorneys for ")
  ...
  for (i in 1:(length(ids)-1)) {
    ...
    party <- html %>% html_table() %>% pluck(3) %>% slice(ids[i]:(ids[i+1]-1)) %>% filter(str_detect(X1, "Attorneys for ") | str_detect(X1, "^Other")) %>% pluck("X1") %>% str_remove("Attorneys for ")
    names <- html %>% html_table() %>% pluck(3) %>% slice(ids[i]:(ids[i+1]-1)) %>% filter(str_detect(X1, "Party name: ")) %>% pluck("X1") %>% str_remove("Party name: ")
    attys <- html %>% html_table() %>% pluck(3) %>% filter(X1 != "") %>% slice(ids[i]:(ids[i+1]-1)) %>% filter(!is.na(X1)) %>% filter(str_detect(X1, "Attorneys for ", negate = TRUE) & str_detect(X1, "Other", negate = TRUE) & str_detect(X1, "Party name: ", negate = TRUE)) %>% pluck("X1")
    parties <- tibble(names = names, attys = attys) %>% nest(.by = names, .key = "attys") %>% bind_cols(party = party)
